...
python -u dataflow-generate_embeddings_to_numpy.py \
    --input_path "$GCS_PATH/labels/data_set.csv" \
    --output_path "$GCS_PATH/inception_resnet/preproc/numpy/" \
    --embedding_levels "PreLogitsFlatten" \
    --filesystem_depth=1 \
    --cloud \
    --job_name "generate-numpy" \
    --setup_file ./setup.py \
